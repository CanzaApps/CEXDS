...
async function getConfiguration(contract) {
  ...
  const config = {
    ...
    universalVoters: [
      "0x12AeAc64bd7E2D1e3b6904059D5759fd015d0a0F",
      "0x78094C71333559E2f150A1B6C404bD02ba0A8AC4",
      "0x8C7Bd9E1977029FA0D97e448e7888d35BEe91576",
      ...
      "0x92bA7219b41794F6023129A593D7715AFd83e2FF"
    ],
    ownedSwaps: [
      {
        entityName: "Binance",
  ...
  if (contract === "cexdefaultswap") {
    ...
    for (const conf of thirdPartySwapConfigs) {
      ...
      if (!conf.tokenAddress) {
        conf.tokenAddress = (
          await (
            await (await hre.ethers.getContractFactory("ERC20Mock")).deploy()
          ).deployed()
        ).address;
      }
...
async function main(params, hre) {
  ...
  if (runOpts.reuse) {
    ...
    if (!reuseAddresses.controller) {
      console.log(
        "No previous controller deployment found. Forcing reuse to false"
      );
      runOpts.reuse = false;
    }
    if (
      !reuseAddresses.oracle &&
      runOpts.startAt !== "oracle" &&
      runOpts.reuse
    ) {
      console.log(
        "No previous oracle deployment found. Forcing startAt to oracle"
      );
      runOpts.startAt = "oracle";
    }
    if (
      !reuseAddresses.voting &&
      runOpts.startAt === "cxdefaultswap" &&
      runOpts.reuse
    ) {
      console.log(
        "No previous Voting deployment found. Forcing startAt to Voting"
      );
  ...
  try {
    ...
    const cexDeployer = await hre.ethers.getContractAt(
      "SwapController",
      reuseAddresses.controller
    );

    if (
      (runOpts.reuse && runOpts.startAt !== "cxdefaultswap") ||
      !runOpts.reuse
    ) {
    ...
    for (const swap of swapsToCreate) {
      let txSwap;
      if (!runOpts.redeployAllSwaps && swap.id in reuseAddresses.swaps)
        continue;
      if (!swap.isThirdParty) {
        txSwap = await cexDeployer.createSwapContract(
          ...
          swap.intialEpochDays.toString(),
          swap.withVoterConsensus
        );
      } else {
        txSwap = await cexDeployer.createSwapContractAsThirdParty(
          swap.entityName,
          swap.entityUrl,
          swap.tokenAddress,
          (swap.premium * 10000).toString(),
...
function readCommandArgs(options) {
  ...
  const startAt = ["controller", "oracle", "voting", "cxdefaultswap"].includes(
    options?.startat
  )
    ? options?.startat
    : "cxdefaultswap";
  const redeployAllSwaps =
    startAt !== "cxdefaultswap" ? true : options?.redeployallswaps || true;
